...
def post_show(request, post_id):
    posts = Post.objects.filter(genre='メニュー').order_by('published_date')
...
def top(request):
    ...
    postnew = Post.objects.filter(genre='その他')
    return render(request, 'museum/top.html', {'posts': posts, 'newss': newss, 'postnew': postnew})
